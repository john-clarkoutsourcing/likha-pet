...
class TraitSystem {
  ...
  String describe(Trait trait) {
    ...
    return switch (e.type) {
      ...
      EffectType.aoe         => '${prefix}Deal ${e.value} AoE damage to ${_targetLabel(e.target)}.',
      EffectType.heal        => '${prefix}Restore ${e.value} HP to ${_targetLabel(e.target)}.',
      EffectType.shield      => '${prefix}Grant ${e.value} shield to ${_targetLabel(e.target)}.',
      EffectType.shieldBreak => '${prefix}Break enemy shield + grant ${e.value} shield to self.',
      EffectType.buff        => '${prefix}${_buffLabel(e)} to ${_targetLabel(e.target)} for ${e.duration} rounds.',
      EffectType.debuff      => '${prefix}${_debuffLabel(e)} on ${_targetLabel(e.target)} for ${e.duration} rounds.',
    };
  }
  ...
  static final Map<String, Trait Function()> _registry = {
    'bakunawa_swallow':  () => TraitLibrary.bakunawaSwallow,
    'lakan_counter':     () => TraitLibrary.lakanCounter,
    'amihan_veil':       () => TraitLibrary.amihanVeil,
    'sarimanok_aura':    () => TraitLibrary.sarimanokAura,
    'tikbalang_charge':  () => TraitLibrary.tikbalangCharge,
    'manananggal_drain': () => TraitLibrary.manananggalDrain,
    'anak_ng_lupa_slam': () => TraitLibrary.anakngLupaSlam,
    'diwata_blessing':   () => TraitLibrary.diwataBlessing,
    'bayanihan_shield':  () => TraitLibrary.bayanihanShield,
    'kapre_smoke':       () => TraitLibrary.kapreSmoke,
    'enkanto_flash':     () => TraitLibrary.enkantoFlash,
    'aswang_fang':       () => TraitLibrary.aswangFang,
    'tikbalang_snipe':   () => TraitLibrary.tikbalangSnipe,
    // Phase 2 skills
    'sigbin_shadow':     () => TraitLibrary.sigbinShadow,
    'nuno_regen':        () => TraitLibrary.nunoRegen,
    'perlas_strike':     () => TraitLibrary.perlasStrike,
    'bathala_wrath':     () => TraitLibrary.bathalaWrath,
  ...
  String _debuffLabel(TraitEffect e) => switch (e.debuffType) {
    ...
    DebuffType.burned      => 'Inflict Burn (${e.value} dmg/round, ignores shield)',
